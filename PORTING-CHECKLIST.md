...
> The `veusz_mathjax.py`, `src/`, `tools/`, `test/`, `fonts/` and `data/`
> citations below are to the **sibling project** `veusz-mathjax-plugin`, the
> released plugin this platform was separated from and is measured against.
> Everything cited without those prefixes is in this repository.

Scope: everything the released plugin does, so the port to
`features/mathjax/feature.js` forgets nothing.
...
| `features/mathjax/feature.js` | **194** (the task says 190; the file is 194 lines) |

Supporting files read for the gap checklist only: `features/mathjax/fonts.js` (20 —
since deleted, because the font list now comes out of the bundle's own declaration),
`.../mathjax/README.md` (119), `jsapi.js` (169), `veusz_js_engine.py`
(lines 1225-1464), `data/fonts.json` (126), `features/mathjax/mathjax.js` (header +
API regions).
...
## 7. Gap checklist against `features/mathjax/feature.js`
...
2. `[MISSING]` Switch the bundle's math font when the chooser changes (and back to the default for an
   ...
   `globalThis.setFont` / `currentFont` (`features/mathjax/mathjax.js`, API region) and
